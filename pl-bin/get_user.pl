	...
	#			print "\n\nUser...(sleep = ".$time_sleep.")";
			...
			if(1){
				...
				if(! (-e $file_tmp) || -z $file_tmp){
					...
					close(F_LOCK);
					$wait = 0;
					while(!-e $lock_file && $wait < 100){
						sleep(1);
						$wait++;
					}

					if($proxy){
						system("wget --load-cookies \"pl-bin/cks.txt\" -t 1 -T 30 -q -Y on --referer=\"http://www.google.com\" -U \"Mozilla/5.0\" -O $file_tmp \"$URL_user\"");
					}else{
						system("wget --load-cookies \"pl-bin/cks.txt\" -t 1 -T 30 -q --referer=\"http://www.google.com\" -U \"Mozilla/5.0\" -O $file_tmp \"$URL_user\"");
					}
				}
					...
					if(! -e $file_user || -z $file_user){
						...
						while($line=<F_USR>){
							...
							if($line ne ""){
								$posx = index($line, ":");
								if($posx > -1){
									$titlex = substr($line, 0, $posx+1);
									$valuex = substr($line, $posx+1, length($line)-1);
...
# ----------------------Friends--------------------------
			...
			if( 1){
				...
				if(! (-e $file_tmp2) || -z $file_tmp2){
					...
					if($proxy){
						system("wget --load-cookies \"pl-bin/cks.txt\" -t 1 -T 30 -q -Y on --referer=\"$URL_user\" -U \"Mozilla/5.0\" -O $file_tmp2 \"$URL_friend\"");
					}else{
						system("wget --load-cookies \"pl-bin/cks.txt\" -t 1 -T 30 -q --referer=\"$URL_user\" -U \"Mozilla/5.0\" -O $file_tmp2 \"$URL_friend\"");
					}					
					...
					if($pos > -1){
						...
						while($tmp_friend_num > $total_friend ){
							...
							if(! (-e $file_tmp3) || -z $file_tmp3){
								...
								if($proxy){
									system("wget --load-cookies \"pl-bin/cks.txt\" -t 1 -T 30 -q -Y on --referer=\"$URL_friend_old\"  -U \"Mozilla/5.0\" -O $file_tmp3 \"$URL_friend2\"");
								}else{
									system("wget --load-cookies \"pl-bin/cks.txt\" -t 1 -T 30 -q --referer=\"$URL_friend_old\" -U \"Mozilla/5.0\" -O $file_tmp3 \"$URL_friend2\"");
								}
								}
